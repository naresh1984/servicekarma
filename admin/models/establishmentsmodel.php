...
class EstablishmentsModel extends Model
{
	...
	public function insertEstablishment($est_name,$email,$city,$phone,$address,$state,$zipcode,$website,$logo,$user_id,$status)
	{
		...
		$sql_est="INSERT INTO establishments(establishment_name,profile_id,is_proof_required,created_date,created_by,modified_by,status) Values('$est_name','$profile_id','Yes',NOW(),'$user_id','$user_id',',$status')";
		$this->_setSql($sql_est);
	    $establishment_id = $this->insertRow();
		
		$sql_categories="INSERT INTO establishment_categories(category_name, master_catrgory_id, status, created_by, created_date, establishment_id) SELECT category_name, id, status, 1, NOW(), $establishment_id FROM categories WHERE status='Active'";
		$this->_setSql($sql_categories);
	    $this->insertRow();	
	...
	public function getAllEstablishments()
	{
		...
		foreach($res as $result):
		 $res[$i]['users_count']=$i;
		 $id = $res[$i]['est_id'];
		 $sql="SELECT count(*) as user_count FROM users WHERE establishment_id = $id and role_id=2";
		 $this->_setSql($sql);
		 $res_1 = $this->getRow();	
	...
	public function EditEstablishment($est_name,$email,$city,$phone,$address,$state,$zipcode,$website,$logo,$user_id,$id,$profile_id,$status){
		...
		$sql_est="UPDATE establishments SET establishment_name='$est_name', is_proof_required='Yes',modified_by='$user_id',status='$status' WHERE profile_id=$profile_id";
		$this->_setSql($sql_est);
	    $this->updateRow();
	}
	...
	{
		 $sql="SELECT *,profiles.state_id as state_id,profiles.city_id as city_id,users.id as id,users.profile_id as profile_id From users INNER JOIN profiles ON users.profile_id = profiles.id INNER JOIN cities ON cities.id = profiles.city_id INNER JOIN states ON states.id = profiles.state_id WHERE users.id = ".$user_id;
		$this->_setSql($sql);
		$res = $this->getAll();
		return $res;
	}
