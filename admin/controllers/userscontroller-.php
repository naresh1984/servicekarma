...
class UsersController extends Controller
{
	public function __construct($model, $action)
	{
		...
		{
			header("Location:".BASE_ADMIN_URL."establishments/index/redirect=".$_REQUEST['load']);
		}		
	}
	...
	public function add()
	{
		...
		{			
			...
			if(move_uploaded_file(@$_FILES["file"]["tmp_name"],"upload/" .$filename. $_FILES["file"]["name"]))
			...
			$user_insert = $this->_model->insertUser($first_name,$last_name,$email,$establishment_id,$city,$phone,$address,$state,$zipcode,$picture,$user_id,$status);
...
/*			if(@$_REQUEST['image_url']!='')
			...
			$id = @$_REQUEST['id'];
			$status = @$_REQUEST['status'];	
			$user_insert = $this->_model->editUser($first_name,$last_name,$email,$establishment_id,$city,$phone,$address,$state,$zipcode,$picture,$user_id,$id,$profile_id,$status);
