...
<!-- Tab menu start - mahesh --> 
<link rel="stylesheet" href="<?php echo BASE_FRONTEND_URL; ?>admin/includes/css/jquery-ui.css">
<script src="<?php echo BASE_FRONTEND_URL; ?>admin/includes/js/jquery-ui.js"></script>
...
<!-- Data Tables start - html -->
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            jQuery("#example tfoot input").keyup(function() {
                /* Filter on the column (the index) of this element */
                oTable.fnFilter(this.value, jQuery("#example tfoot input").index(this));
            });
            
            if($("#example tfoot").is(':visible')){
            jQuery("#example tfoot input").each(function(i) {
                //asInitVals[i] = this.value;
            });
            }


            jQuery("#example tfoot input").focus(function() {
            ...
            jQuery("#example tfoot input").blur(function(i) {
                if (this.value == "")
                {
                    this.className = "search_init";
                    this.value = asInitVals[jQuery("#example tfoot input").index(this)];
                }
            });
		   ...
		   var oTable2 = $('#example2').dataTable({
            ...
            jQuery("#example2 tfoot input").keyup(function() {
                /* Filter on the column (the index) of this element */
                oTable2.fnFilter(this.value, jQuery("#example2 tfoot input").index(this));
            });
            
            if($("#example2 tfoot").is(':visible')){
            jQuery("#example2 tfoot input").each(function(i) {
                //asInitVals[i] = this.value;
            });
            }


            jQuery("#example2 tfoot input").focus(function() {
                if (this.className == "search_init")
                {
                    this.className = "";
                    this.value = "";
            ...
            jQuery("#example2 tfoot input").blur(function(i) {
                if (this.value == "")
                {
                    this.className = "search_init";
                    this.value = asInitVals[jQuery("#example2 tfoot input").index(this)];
                }
            });
...
<div class="wrapper-910">     
    <div id="tabs">
        <ul>
            <li><a href="#tabs-1">Users Reports</a></li>
            <li><a href="#tabs-2">Establishments Reports</a></li>            
        </ul>
        <div id="tabs-1">            
        ...
        <div id="tabs-2">
            ...
            <div class="clear"></div> 
            <div>
                <table  id="example2"  class="display" style="text-align:left">
                    <thead>
                ...
                <?php 
                   // echo "<pre>";
                    //print_r($allEstablishments);
                    $i = 1; 			
                    foreach($allEstablishments as $estblish):
                    if($i%2==0)
                     $class = 'even';
                    else
                      $class = 'odd';
                ?>
                  <tr class="even_gradeC <?php echo $class; ?>">
                      <td align="center"><input type="checkbox" value="<?php echo $estblish['est_id']; ?>" name="check17"></td>
                      <td valign="middle" align="left"><?php echo $estblish['establishment_name']; ?></td>
                      <td valign="middle"><?php echo $estblish['phone']; ?></td>
                      <td valign="middle"><?php echo $estblish['state']; ?></td>
                      <td valign="middle"><?php echo $estblish['city']; ?></td>
                    <!--  <td valign="middle"><?php //echo  wordwrap(@$estblish['street1'], 25, "<br />\n"); ?></td>-->
                      <td valign="middle"><?php echo @$estblish['required_hours']; ?></td>
                      <td><?php echo @$estblish['users_count']; ?></td>                    
                    </tr>
                <?php endforeach; ?>    
                  </tbody>
           	 </table>
