...
function approve_reject_assignment(str){
	   var error = 0;
	   var approve_status = str;
	   if(approve_status=="reject" || approve_status=="rejectemail")
	   {
		   	$('#reject_reason_div').show();
			if($.trim($('#rejected_reason').val()) == ""){
				if(!$('#rejected_reason').hasClass('form-error')){
					$('#rejected_reason').addClass('form-error').after('<div id="rejected_reason_error" class="error-message">Please enter Rejected Reason</div>');
				}
				error++;
			} else {
				$('#rejected_reason').removeClass('form-error');
				$('#rejected_reason_error').remove();
			}	
	   }	   	
	   	if(error){
		return false;
		}else{	  
		var url = '../assignments/approve';		
		$.ajax({
		   type: "POST",
		   url: url,
		   data: $('#form_assignments').serialize()+"&approve_status="+approve_status+"&ajaxcheck=1&xxxxx="+Math.random(),
		   success: function(result){	
		  	   alert(result);	
			   if(result=='2'){
				  	$('#rejected_reason_error').remove();
