...
class UsersModel extends Model
{
	...
	{
		$sql="SELECT * FROM establishments WHERE status!='Delete' AND id = ".@$_SESSION['user_details'][0]['establishment_id']." ORDER BY establishment_name ASC";
		$this->_setSql($sql);
		$res = $this->getAll();	
		return $res;					
	}
	...
	public function getAllUsers($select_establishment)
	{
		...
		$sql="SELECT *,users.id as user_id,users.status as user_status,profiles.id as profile_id FROM users INNER JOIN profiles ON profiles.id = users.profile_id INNER JOIN states ON states.id = profiles.state_id INNER JOIN cities ON cities.id = profiles.city_id INNER JOIN establishments AS est ON est.id = users.establishment_id ".$clause." WHERE users.establishment_id = ".@$_SESSION['user_details'][0]['establishment_id']." ORDER BY users.username ASC";
