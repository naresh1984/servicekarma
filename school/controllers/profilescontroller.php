...
class ProfilesController extends Controller
{
	...
	public function editEst()
	{
	   ...
	   // @$id = @$string[2];
		 @$id = $_SESSION['user_details'][0]['establishment_id'];
		if(@$_REQUEST['ajaxcheck']==1)
		{
		...
		}else if(!isset($_REQUEST['ajaxcheck']) && trim(@$_REQUEST['est_name'])!='' && trim(@$_REQUEST['est_name'])!='')
		{
			...
			$logo="";
			$fname = date("m-d-Yhms");
			if(move_uploaded_file(@$_FILES["file"]["tmp_name"],"../admin/upload/" .$fname. $_FILES["file"]["name"])){
            $logo = "upload/" .$fname. $_FILES["file"]["name"];
			if(@$_REQUEST['image_url']!='')
			  unlink("../admin/".$_REQUEST['image_url']);
			}
			...
			$est_insert = $this->_model->EditEstablishment($est_name,$email,$city,$phone,$address,$state,$zipcode,$required_hours,$website,$logo,$user_id,$id,$profile_id,$status);
